...
describe("SwapExactETHForTokens contract", function () {
  ...
  describe("Transactions", function () {
    ...
    describe("Setters", function () {
      it("Only owner should set the recipient", async function () {
        hardhatSwapExactETHForTokens = await SwapExactETHForTokens.deploy(
          UNISWAP_ROUTER_ADDRESS,
          DAI_TOKEN_ADDRESS,
          owner.address,
        ...
        await hardhatSwapExactETHForTokens.setRecipient(acc2.address);

        expect(await hardhatSwapExactETHForTokens.getRecipient()).to.equal(
          acc2.address
        );

        await expect(
          hardhatSwapExactETHForTokens
            .connect(acc1)
            .setRecipient(accs[0].address)
        ).to.be.revertedWith("revert Ownable: caller is not the owner");
      });

      it("Only owner should set the exchange", async function () {
        hardhatSwapExactETHForTokens = await SwapExactETHForTokens.deploy(
          UNISWAP_ROUTER_ADDRESS,
          DAI_TOKEN_ADDRESS,
          owner.address,
        ...
        await hardhatSwapExactETHForTokens.setExchange(acc2.address);

        expect(await hardhatSwapExactETHForTokens.getExchange()).to.equal(
          acc2.address
        );
        ...
        await expect(
          hardhatSwapExactETHForTokens
            .connect(acc1)
            .setExchange(accs[0].address)
        ).to.be.revertedWith("revert Ownable: caller is not the owner");
      });

      it("Only owner should set the exchange percentage", async function () {
        hardhatSwapExactETHForTokens = await SwapExactETHForTokens.deploy(
          UNISWAP_ROUTER_ADDRESS,
          DAI_TOKEN_ADDRESS,
          owner.address,
        ...
        await hardhatSwapExactETHForTokens.setExchangePercentage(20);

        expect(
          await hardhatSwapExactETHForTokens.getExchangePercentage()
        ).to.equal(20);

        await expect(
          hardhatSwapExactETHForTokens.connect(acc1).setExchangePercentage(100)
        ).to.be.revertedWith("revert Ownable: caller is not the owner");
      });
    });
    ...
    describe("receive", function () {
      it("Should swap exact ETH for tokens and send all to owner", async function () {
        ...
        const ownerIntialDAIBalance = await daiContract.balanceOf(
          owner.address
        );
    ...
    describe("withdrawTokens", function () {
      it("Should withdraw all tokens from contract and send it to owner", async function () {
        const acc2InitialETHBalance = await acc2.getBalance();
        const ownerIntialDAIBalance = await daiContract.balanceOf(
          owner.address
        );
        ...
        await hardhatSwapExactETHForTokens.setRecipient(
          hardhatSwapExactETHForTokens.address
        );
        expect(await hardhatSwapExactETHForTokens.getRecipient()).to.equal(
          hardhatSwapExactETHForTokens.address
        ...
        const contractIntialDAIBalance = await daiContract.balanceOf(
          hardhatSwapExactETHForTokens.address
        );
        ...
        let daiInContract = await daiContract.balanceOf(
          hardhatSwapExactETHForTokens.address
        );
        let recipientDai = await daiContract.balanceOf(owner.address);
        const exchangeDai = await daiContract.balanceOf(acc1.address);

        console.log(
          `🚀 ${fromEther} ETH -> 
        ${ethers.utils.formatEther(daiInContract.sub(ownerIntialDAIBalance))} + 
        ${ethers.utils.formatEther(exchangeDai.sub(acc1IntialDAIBalance))} DAI`
        );

        expect(await acc2.getBalance()).to.be.below(acc2InitialETHBalance);
        expect(daiInContract).to.be.above(contractIntialDAIBalance);
        expect(recipientDai).to.equal(ownerIntialDAIBalance);
        expect(exchangeDai).to.be.equal(acc1IntialDAIBalance);

        await hardhatSwapExactETHForTokens.withdrawTokens(DAI_TOKEN_ADDRESS);
        daiInContract = await daiContract.balanceOf(
          hardhatSwapExactETHForTokens.address
        );
        recipientDai = await daiContract.balanceOf(owner.address);
        expect(daiInContract).to.equal(0);
        expect(recipientDai).to.be.above(contractIntialDAIBalance);
      });
    });

    describe("withdrawEthers", function () {
      it("Should withdraw all ETH from contract and send it to owner", async function () {
        // TODO: find a way to send ETH to contract and not trigger the swap
        ...
        // );
        // await hardhatSwapExactETHForTokens.deployed();
        // await hardhatSwapExactETHForTokens.withdrawEthers();
        // expect(
        //   await provider.getBalance(hardhatSwapExactETHForTokens.address)
        // ).to.equal(0);
      });
